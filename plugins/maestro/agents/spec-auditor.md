...
1. **Antes de investigar qualquer coisa**, grave `.maestro/tmp/verdicts/descoberta-spec.md` com `veredito: EM_ANDAMENTO` e a lista de checagens que você pretende fazer, todas desmarcadas.
2. Termine toda a investigação — comandos, leituras, capturas. Não sobra nenhuma verificação para depois.
3. **Sobrescreva** `.maestro/tmp/verdicts/descoberta-spec.md` com o veredito real, no formato abaixo.
...
A única exceção: se uma correção alterou o **escopo do Release 1** no PRD, a rastreabilidade inteira volta a valer e você a refaz. Mudança de escopo invalida a passada anterior.
...
- O escopo declarado como fora do Release 1 não reapareceu como tela
