...
- **Se falhar**: retorna diretamente ao Executor original (Backend ou Frontend Engineer) com o erro exato — **não gera decline payload formal**, pois o erro estático já é autoexplicativo (arquivo + linha + mensagem do compilador/linter). Este é o único gate cuja rejeição não passa pelo protocolo de payload em `.maestro/tmp/`.
