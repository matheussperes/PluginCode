...
Rode `.maestro/scripts/bootstrap.sh` para garantir que `node`/`npm` estão disponíveis e que as devDependencies necessárias (`@supabase/supabase-js`, `typescript`, `ts-node`, `dotenv`) estão instaladas.
...
Antes de qualquer merge, a task passa por até três fiscalizadores: Code Auditor (build/lint), Security Auditor (secrets/RLS/OWASP) e UX Auditor (validação visual, usando `.maestro/scripts/seed-qa-user.ts` para autenticar um usuário de teste no Supabase quando a tela exige login). Qualquer um dos três pode vetar.
...
Após o merge, o Memory Manager atualiza `Backlog.md`/`Status.md` automaticamente. Ao final de cada sprint, o Improvement Agent registra aprendizados em `docs/Lessons-Learned.md` e sincroniza com `.maestro/scripts/sync-lessons.sh`.
